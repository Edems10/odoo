...
from ..exception.exceptions import EnrollmentError,CourseStateError,CourseStateError
...
class CourseSecurityService:
    ...
    def can_edit_course_details(self, course, user=None):
        """Check if user can edit course details."""
        user = user or self.env.user
        return (user.has_group(CourseConstants.Security.ADMIN_GROUP) or 
                course.teacher_id == user)
    
    def can_edit_price(self, course, user=None):
        """Check if user can edit course price."""
        return self.can_edit_course_details(course, user)
    
    def can_edit_teacher(self, course, user=None):
...
class CourseStateService:
    ...
    def close_enrollment(self, course):
        ...
        if course.state != CourseConstants.States.PUBLISHED:
            raise CourseStateError(_("Can only close enrollment for published courses."))
        course.state = CourseConstants.States.CLOSED
    
    def reopen_enrollment(self, course):
    ...
    def set_to_draft(self, course):
        """Set course back to draft."""
        if course.student_ids:
            raise CourseStateError(
                _("Cannot set course '%s' to draft because it has enrolled students.") % course.name
            )
